module.exports = function(grunt){
  ...
  grunt.registerTask('build', 'Build the add-on', function() {
    ...
    grunt.util.spawn({
      ...
      args: ['xpi',
        '--update-link',
        'https://people.mozilla.com/~bwinton/quick-load/quick-load.xpi',
        '--update-url',
        'https://people.mozilla.com/~bwinton/quick-load/quick-load.update.rdf'
      ]
  ...
  grunt.registerTask('copy', 'Copy the files to the server.', function() {
    ...
    if (!grunt.file.exists('/Volumes/People/public_html/quick-load')) {
      grunt.log.error('Missing Directory!');
      done(false);
      return;
    }

    grunt.file.copy('quick-load.xpi',
                    '/Volumes/People/public_html/quick-load/quick-load.xpi',
                    {"encoding": null});
    grunt.log.ok('Copied XPI…');
    grunt.file.copy('quick-load.update.rdf',
                    '/Volumes/People/public_html/quick-load/quick-load.update.rdf',
                    {"encoding": null});
    grunt.log.ok('Copied update.rdf…');
    grunt.file.copy('index.html',
                    '/Volumes/People/public_html/quick-load/index.html',
                    {"encoding": null});
